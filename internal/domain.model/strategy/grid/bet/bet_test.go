...
func TestBetData_WriteToJSON(t *testing.T) {
    gridBet := NewGridBet()
    gridBet.Grid.LoadFromJSON(context.Background())
    originStep := gridBet.Grid.RunBet.Step
    gridBet.Grid.RunBet.Step = originStep + 1
    r := gridBet.Grid.WriteToJSON(context.Background())
    assert.True(t, r)
    gridBet.Grid.LoadFromJSON(context.Background())
    assert.Equal(t, originStep+1, gridBet.Grid.RunBet.Step)
    gridBet.Grid.RunBet.Step = originStep
